...
https://www.bilibili.com/video/BV1eJ4m157kC/?spm_id_from=333.1391.0.0&vd_source=c5b001da49827c95685ff66b3392ffc9
...
既然是语言，那就会有相应的编辑器，例如Typora、VSCode等等。我使用的方法是VitePress+Node.js+npm本地实时预览，如果能接受记事本就暂时不用下载其他东西。

下载编辑器对管理更方便，例如VSCodo可以直接对文件夹内的文件操作（很方便），文本也会给你标颜色
...
在`.md`文件中引入`.pdf`文件：这里使用相对地址，把文件放到专门的文件夹
...
加上`{target="_blank"}`实现新窗口打开（VitePress支持）

::: warning
在本地测试的时候可以正常打开，托管到Github之后就打不开了（文件正常上传）
:::
