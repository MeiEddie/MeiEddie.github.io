...
export default defineConfig({
  ...
  themeConfig: {
    nav: [
      ...
      { text: '游乐场', link: '/游乐场' },
      // 友链页，排在「游乐场」右侧（2026.9.23 与游乐场互换过位置）。
      // ⚠️ 没在下面 sidebar 里配 key → 该页不显示侧边栏（与「文章专区」一致）。
      // ⚠️⚠️ 结尾**绝对不能加斜杠**（2026.9.24 二十八轮改版：口径与 9.21 相反）。
      //     2026.9.24 起页面文件从 docs/友链/index.md 提成了 **docs/友链.md**，
      //     路由随之由 /友链/ 变成 /友链，产物是 友链.html。
      //     写成 '/友链/' 会解析不到页面 → 线上 404（与 9.21 那次正好反过来）。
      //     所以：目录型页面（有 index.md）才带斜杠，单文件页面**不带** —— 别凭记忆改回去。
      { text: '友链', link: '/友链' },
    ],
